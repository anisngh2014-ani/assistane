...
export default function SupportCodePanel({ accountId = null }) {
  ...
  const copyLink = (shortCode) => {
    ...
    const url = `${window.location.origin}/connect?code=${shortCode}`;
    navigator.clipboard.writeText(url);
    setCopied(shortCode);
    toast({ title: "Link copied!", description: "Share the link with your client." });
    setTimeout(() => setCopied(null), 2000);
  };
  ...
  const shareLink = (shortCode) => {
    const url = `${window.location.origin}/connect?code=${shortCode}`;
    if (navigator.share) {
      navigator.share({ title: "Support Link", text: `Your support code is ${shortCode}`, url });
      setCopied(shortCode);
      setTimeout(() => setCopied(null), 2000);
    } else {
      copyLink(shortCode);
  ...
  return (
    <div className="bg-card border border-border rounded-xl p-5 space-y-4">
      ...
      <p className="text-xs text-muted-foreground">
        Generate a 6-digit support code to share with your client. They can paste it directly into the Agent, or open the support link and follow the download steps.
      </p>
