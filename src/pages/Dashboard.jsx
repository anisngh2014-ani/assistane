...
export default function Dashboard() {
  ...
  useEffect(() => {
    const load = async () => {
      const [me, devicesRes, sess] = await Promise.all([
        assistane.auth.me(),
        assistane.functions.invoke("deviceApi", { endpoint: "devices" }),
        assistane.entities.Session.list("-session_start", 10),
      ]);
      setUser(me);
  ...
  useEffect(() => {
    const unsubscribe = assistane.entities.Device.subscribe((event) => {
  ...
  const handleConnect = (device) => launchNativeViewer({ assistane, device, toast });

  const handleDeleteDevice = async (device) => {
    if (!window.confirm(`Delete ${device.device_name}? This cannot be undone.`)) return;
    try {
      await assistane.entities.Device.delete(device.id);
