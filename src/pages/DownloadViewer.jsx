...
const VIEWER_DOWNLOADS = {
  windows: "https://github.com/anisngh2014-ani/assistane/releases/download/viewer-latest/Assistane%20Viewer%20Setup.exe",
  macos: "https://github.com/anisngh2014-ani/assistane/releases/download/viewer-latest/Assistane%20Viewer.dmg",
};
...
export default function DownloadViewer() {
  ...
  const startDownload = (platform) => {
    const downloadUrls = {
      windows: VIEWER_DOWNLOADS.windows,
      macos: VIEWER_DOWNLOADS.macos,
    };
  ...
  return (
    <div className="max-w-2xl mx-auto py-16 px-4">
      <div className="bg-card border border-border rounded-2xl p-8 text-center space-y-6">
        ...
        <div>
          <h1 className="font-heading font-bold text-2xl mb-2">Download Assistane Viewer</h1>
          <p className="text-muted-foreground">Remote desktop client for control & monitoring</p>
        </div>
        ...
        <div className="bg-secondary/50 rounded-lg p-4 text-left space-y-2 text-xs text-muted-foreground">
          <p>✓ View remote desktop in real-time</p>
          <p>✓ Control mouse, keyboard & screen</p>
          <p>✓ Send files & manage devices</p>
        </div>
      </div>
    </div>
  );
