...
async function launchNativeViewer({ assistane, device, accountId, toast }) {
  if (!device) return;
...
export default function OwnerDashboard() {
  ...
  const [savingOwnerPassword, setSavingOwnerPassword] = useState(false);
  ...
  const invokeWithRetry = async (functionName, payload, attempts = 2) => {
    ...
    for (let i = 0; i < attempts; i++) {
      try {
        return await assistane.functions.invoke(functionName, payload);
      } catch (err) {
        lastError = err;
  ...
  const load = async () => {
    setLoading(true);
    try {
      const me = await assistane.auth.me();
      setUser(me);
  ...
  const handleOwnerPasswordChange = async (event) => {
    event.preventDefault();
    if (!currentOwnerPassword || !newOwnerPassword) {
      toast({ title: "Enter current and new password" });
      return;
    }
    ...
    setSavingOwnerPassword(true);
    try {
      const res = await assistane.functions.invoke("owner-change-password", {
        endpoint: "owner-change-password",
        current_password: currentOwnerPassword,
        new_password: newOwnerPassword,
      });
      if (res.data.success) {
        toast({ title: "Owner password updated" });
  ...
  const ownerDevices = devices.filter((d) => d.user_id === user?.id);

  const filteredDevices = ownerDevices.filter((d) =>
  ...
  const filteredSessions = sessions.filter((s) =>
    ...
    s.account_username?.toLowerCase().includes(search.toLowerCase()) ||
    s.account_name?.toLowerCase().includes(search.toLowerCase()) ||
    s.account_email?.toLowerCase().includes(search.toLowerCase())
  );

  const activeUsers = users.filter((u) => u.status === "active").length;
  const activeDevices = ownerDevices.filter((d) => d.online_status === "online").length;
  ...
  const metrics = [
    ...
    { label: "Total Devices", value: ownerDevices.length, icon: Monitor, accent: "text-violet-400 bg-violet-500/10" },
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
          <form onSubmit={handleOwnerPasswordChange} className="bg-card border border-border rounded-xl max-w-sm w-full p-6 space-y-4">
            ...
            <div className="flex gap-2">
              ...
              <Button type="submit" className="flex-1 h-9" disabled={savingOwnerPassword}>
                {savingOwnerPassword ? "Saving..." : "Save"}
              </Button>
            </div>
          </form>
        </div>
                     ...
                     >
                      ...
                      <div className="flex items-center justify-end gap-2">
                        {u.status !== "suspended" ? (
                          <>
                            <button
                              onClick={(e) => { e.stopPropagation(); setNewPassword(""); setEditingId(editingId === u.id ? null : u.id); }}
                              className="inline-flex items-center gap-1 rounded-md px-2 py-1 text-[11px] font-semibold text-amber-300 hover:text-amber-200 hover:bg-amber-500/10 transition"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
                {filteredDevices.map((device) => (
                  <DeviceCard 
                    key={device.id} 
                    device={device} 
                    onConnect={() => launchNativeViewer({ assistane, device, toast })}
                    onDelete={() => {
                      if (!window.confirm(`Delete ${device.device_name}?`)) return;
                      assistane.functions.invoke("deviceApi", {
                        endpoint: "device",
                        id: device.id,
                        _method: "DELETE",
            ...
            <table className="w-full text-sm">
              <thead className="bg-secondary/50 border-b border-border">
                <tr>
                  ...
                  <th className="px-4 py-2 text-left text-xs font-semibold">Status</th>
                  <th className="px-4 py-2 text-left text-xs font-semibold">Started By</th>
                </tr>
              </thead>
              <tbody>
                {filteredSessions.map((s) => (
                  <tr key={s.id} className="border-b border-border hover:bg-secondary/30">
                    ...
                    <td className="px-4 py-2 text-xs">
                      <div className="font-semibold">{s.account_username || s.account_name || "Owner"}</div>
                      <div className="text-muted-foreground">{s.account_id || s.account_email || s.user_id}</div>
                    </td>
                    ...
                    <td className="px-4 py-2 text-xs">{s.requester_type || "viewer"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
