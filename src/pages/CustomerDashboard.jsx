...
async function launchNativeViewer({ assistane, device, accountId, toast }) {
  ...
  try {
    const res = await assistane.functions.invoke("deviceApi", {
      ...
      account_token: localStorage.getItem("accountToken") || undefined,
    });
    data = res?.data || {};
  } catch (err) {
    toast?.({
...
export default function CustomerDashboard() {
  ...
  const forceLogout = (reason = "suspended") => {
    localStorage.removeItem("accountToken");
    localStorage.removeItem("accountId");
    localStorage.removeItem("accountDbId");
    localStorage.removeItem("accountName");
    localStorage.removeItem("accountEmail");
    navigate(`/user-login?error=${reason}`);
  };
  ...
  const checkAccountStatus = async (id) => {
    try {
      ...
      } else if (res.data.invalid_session) {
        forceLogout("session");
      }
    } catch (_) {}
  };

  const isAccountAccessError = (message = "") => {
    return /account suspended|account not active|invalid account session|missing account_token|unauthorized|401|403/i.test(message);
  };
  ...
  useEffect(() => {
    ...
    const dataInterval = setInterval(() => {
      ...
      const currentId = localStorage.getItem("accountId");
      if (currentToken && currentId) loadData(currentToken, currentId, { silent: true });
    }, 12000);
  ...
  const loadData = async (token, id, options = {}) => {
    if (!options.silent) setLoading(true);
    try {
      const devicesRes = await assistane.functions.invoke("deviceApi", {
        endpoint: "devices",
        account_id: id,
        account_token: token,
      });
      if (devicesRes.data.error && isAccountAccessError(devicesRes.data.error)) {
        forceLogout(devicesRes.data.error === "Account suspended" ? "suspended" : "session");
        return;
      }
      ...
      const sessionsRes = await assistane.functions.invoke("deviceApi", {
        endpoint: "sessions",
        account_id: id,
        account_token: token,
      });
      ...
      } else if (sessionsRes.data.error && isAccountAccessError(sessionsRes.data.error)) {
        forceLogout(sessionsRes.data.error === "Account suspended" ? "suspended" : "session");
        return;
      }
      ...
      if (isAccountAccessError(err.message || "")) {
        forceLogout("session");
      }
    } finally {
      setLoading(false);
  ...
  const handleConnectDevice = (device) => {
    launchNativeViewer({ assistane, device, accountId, toast });
  };
