...
export default function WorkspaceSetup() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setUser(me);
        const ws = await assistane.entities.Workspace.filter({ owner_id: me.id });
        if (ws.length > 0) {
          setWorkspace(ws[0]);
          setFormData(ws[0]);
        }
  ...
  const handleSave = async () => {
    ...
    try {
      if (workspace) {
        // Update existing
        await assistane.entities.Workspace.update(workspace.id, formData);
        toast({ title: "Workspace updated" });
      } else {
        // Create new
        const ws = await assistane.entities.Workspace.create({
          owner_id: user.id,
          ...formData,
        });
        setWorkspace(ws);
