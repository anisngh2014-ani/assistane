...
export default function SupportAssistant() {
  ...
  useEffect(() => {
    loadConversations();
    assistane.entities.Device.list().then(setDevices);
  }, []);
  ...
  const loadConversations = async () => {
    ...
    const user = await assistane.auth.me();
    const data = await assistane.entities.SupportConversation.filter(
      { user_id: user.id },
      "-updated_date",
      50
    );
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const result = await assistane.integrations.Core.InvokeLLM({ prompt });
      ...
      if (activeConv) {
        const updated = await assistane.entities.SupportConversation.update(activeConv.id, {
        ...
        const created = await assistane.entities.SupportConversation.create({
          user_id: user.id,
          title,
  ...
  const handleSummarize = async () => {
    ...
    try {
      ...
      const summary = await assistane.integrations.Core.InvokeLLM({ prompt });
      const updated = await assistane.entities.SupportConversation.update(activeConv.id, { summary });
  ...
  const handleResolve = async () => {
    ...
    const updated = await assistane.entities.SupportConversation.update(activeConv.id, { status: "resolved" });
