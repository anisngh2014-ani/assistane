...
assistane://connect?device_id=xxx&device_name=yyy&...
    ...
    ↓
Assistane Viewer (Electron app, this folder)
    |
...
```json
{
  "apiBaseUrl": "https://api.assistane.com",
  "apiKey": ""
}
```

`app-config.local.json` is ignored by git. For GitHub Actions builds, set repository secrets `ASSISTANE_API_BASE_URL` and `ASSISTANE_API_KEY`.
...
- `https://downloads.assistane.com/viewer/windows/Assistane.Viewer.Setup.exe` (Windows)
- `https://downloads.assistane.com/viewer/macos/Assistane.Viewer.dmg` (macOS)
