...
Deno.serve(async (req) => {
  try {
    ...
    if (path === "devices") {
      ...
      if (accountId) {
        // Account-based access (Customer Dashboard)
        // accountId may be the 6-7 digit account_id OR the UUID entity id
        let account = null;
        const byAccountId = await base44.asServiceRole.entities.Account.filter({ account_id: accountId });
        account = byAccountId[0] || null;
        if (!account) {
          try { account = await base44.asServiceRole.entities.Account.get(accountId); } catch(_) {}
        }
        if (!account) return Response.json({ error: "Account not found", success: false }, { status: 404 });
        
        // Revoke access immediately if account is suspended or deleted
        if (account.status === "suspended" || account.status === "inactive") {
          return Response.json({ error: "Account suspended", success: false }, { status: 403 });
        }
        
        const devices = await base44.asServiceRole.entities.Device.filter({ user_id: account.id });
        ...
        const devices = await base44.asServiceRole.entities.Device.filter({ user_id: user.id });
    ...
    if (path === "sessions") {
      ...
      if (body?.account_id) {
        const account = await findAccount(body.account_id);
        if (!account || account.status !== "active") {
          return Response.json({ error: "Account not active", success: false }, { status: 403 });
        }
        ownerId = account.id;
      } else {
        const user = await base44.auth.me();
    ...
    if (path === "device") {
      ...
      if (deviceId) {
        // Direct device deletion by ID (for account-based customers)
    ...
    if (path === "webrtc-pending") {
      ...
      return Response.json({ pending: true, signal_id: signal.id, offer_sdp: signal.offer_sdp });
    }
    ...
    if (path === "expire-support-code") {
      ...
      if (account_id) {
        const account = await findAccount(account_id);
        if (!account || account.id !== code.user_id) {
    ...
    if (path === "account-login") {
      ...
      try {
        ...
        // Generate a session token (use UUID as temporary token)
    ...
    if (path === "create-signal") {
      const body = await getBody();
      const { device_id, offer_sdp, auth_token } = body;
