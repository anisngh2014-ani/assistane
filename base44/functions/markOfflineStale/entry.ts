...
Deno.serve(async (req) => {
  try {
    ...
    // Mark devices offline if last_seen > 120 seconds ago. The Agent normally
    // heartbeats every 30 seconds, so this tolerates missed heartbeats while
    // still clearing uninstalled/crashed Agents quickly.
    ...
    await Promise.all(
      stale.map((d) =>
        base44.asServiceRole.entities.Device.update(d.id, {
